...
disE  = dis[dis$dates>='1950-01-01' & dis$dates<='2017-12-31',]  
...
disE.mo.ts  = ts(disE.mo$avg,start=c(1950,1),end=c(2017,12),frequency=12)
disE.mo.sum.ts  = ts(disE.mo$sumflow,start=c(1950,1),end=c(2017,12),frequency=12)
...
title(paste(stinfo[1,2],': 1950-2017',sep=''),outer=T)

#--------------------------- duration curves -----------------------------------
#simply converting each vector to 'ts' allows to cbind without recycling values in shorter vector

dis.fdc = as.data.frame(cbind(ts(disE$val[disE$year<2016]),ts(disE$val[disE$year>=2016])))
par(mfrow=c(1,1))
fdc(dis.fdc,ylab='Flow (cf/s)',leg.txt=c('1950-2015','2016-2017'),cex.axis=1)
title(stinfo[1,2],outer=T)
